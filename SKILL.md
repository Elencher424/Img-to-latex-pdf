---
name: img-to-latex-pdf
description: >
  Convert any image containing text, math, tables, or diagrams into a professionally formatted LaTeX document and compile to PDF.
  This skill orchestrates three steps: (1) OCR via local Qwen VL models (Ollama, OpenAI-compatible API, or transformers) to extract text,
  (2) generate a LaTeX document matched to the user's intent (math solution, general OCR, table extraction, formula rendering, document digitization),
  (3) compile to PDF with xelatex.
  Triggers when the user provides an image and asks to "convert to LaTeX/PDF", "solve this math problem", "OCR this image",
  "extract text to PDF", "make a PDF from this image", "用LaTeX做这道题", or any request involving image-to-LaTeX-PDF workflow.
  Also triggers when the user uploads an image of text/math and wants a formatted document, even without explicitly mentioning LaTeX or PDF.
---

# Image → LaTeX → PDF Pipeline

> 📖 [中文版](./SKILL_zh.md) | [README](./README.md)
