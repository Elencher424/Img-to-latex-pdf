...
description: >
  将任何包含文字、数学公式、表格或图表的图片转换为专业排版的 LaTeX 文档并编译为 PDF。
  本 skill 编排三个步骤：(1) 通过本地 Qwen VL 模型（Ollama、OpenAI 兼容 API 或 transformers）进行 OCR 文字提取，
  (2) 根据用户意图生成匹配的 LaTeX 文档（数学解答、通用 OCR、表格提取、公式渲染、文档数字化），
  (3) 使用 xelatex 编译为 PDF。
  当用户提供图片并要求"转换为 LaTeX/PDF"、"解这道数学题"、"OCR 识别这张图片"、
  "提取文字生成 PDF"、"用 LaTeX 做这道题"，或任何涉及图片→LaTeX→PDF 工作流的请求时触发。
  当用户上传包含文字/数学内容的图片并希望生成格式化文档时也会触发，即使未明确提及 LaTeX 或 PDF。
---

# 图片 → LaTeX → PDF 流水线

> 📖 [English](./SKILL.md) | [README](./README.md)

通用流水线：用任意本地 Qwen VL 模型对图片进行 OCR → 生成 LaTeX 文档 → 编译为 PDF。

## 请注意！

本skill并不包含LATEX，Ollama和千问模型
